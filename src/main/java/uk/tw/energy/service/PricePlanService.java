...
public class PricePlanService {
	...
	public BigDecimal calculateMultiplyCost(List<ElectricityReading> electricityReadings, PricePlan pricePlan) {
		BigDecimal average = calculateAverageReadingNotHalfUp(electricityReadings);
		BigDecimal timeElapsed = calculateTimeElapsed(electricityReadings);
	...
	private BigDecimal calculateAverageReadingNotHalfUp(List<ElectricityReading> electricityReadings) {
		BigDecimal summedReadings = electricityReadings.stream()
				.map(ElectricityReading::getReading)
				.reduce(BigDecimal.ZERO, BigDecimal::add);

		return summedReadings.divide(BigDecimal.valueOf(electricityReadings.size()), 2, RoundingMode.HALF_UP);
	}
	...
	public PricePlan getById(String pricePlanId) {
		return Optional.ofNullable(pricePlans).orElseGet(Collections::emptyList)
				.stream()
				.filter(pricePlan -> Objects.equals(pricePlan.getPlanName(), pricePlanId))
				.findFirst()
				.get();
	}
